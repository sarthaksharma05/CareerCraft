...
serve(async (req) => {
  ...
  try {
    ...
    console.log("✅ Using updated ElevenLabs API key for voiceover generation");
    ...
    const { data: profile, error: profileError } = await supabase
      .from('profiles')
      .select('is_pro, usage_limits, usage_counts, elevenlabs_api_connected, api_connections')
      .eq('id', user.id)
      .single();
    ...
    if (!testResponse.ok) {
      ...
      // Update user's API connection status to false
      if (profile) {
        const updatedConnections = {
          ...profile.api_connections,
          elevenlabs: {
            connected: false,
    ...
    // Update user's API connection status to true on successful call
    if (profile) {
      const updatedConnections = {
        ...profile.api_connections,
        elevenlabs: {
